...
func (s *Suite) SetupSuite() {
	...
	r.Run(`kubectl apply -k ../../examples/multiforwarder`)
...
func (s *Suite) TestKernel2Ethernet2Kernel_Vfio2Noop() {
	...
	s.T().Cleanup(func() {
		r.Run(`NSE_VFIO=$(kubectl get pods -l app=nse-vfio -n ns-kernel2ethernet2kernel-vfio2noop --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	...
	r.Run(`kubectl apply -k ../../../examples/use-cases/Kernel2Ethernet2Kernel_Vfio2Noop`)
	...
	r.Run(`NSC_KERNEL=$(kubectl get pods -l app=alpine -n ns-kernel2ethernet2kernel-vfio2noop --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	r.Run(`NSE_KERNEL=$(kubectl get pods -l app=nse-kernel -n ns-kernel2ethernet2kernel-vfio2noop --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	r.Run(`NSC_VFIO=$(kubectl get pods -l app=nsc-vfio -n ns-kernel2ethernet2kernel-vfio2noop --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	...
	r.Run(`kubectl exec ${NSC_KERNEL} -n ns-kernel2ethernet2kernel-vfio2noop -- ping -c 4 172.16.1.100`)
	r.Run(`kubectl exec ${NSE_KERNEL} -n ns-kernel2ethernet2kernel-vfio2noop -- ping -c 4 172.16.1.101`)
...
func (s *Suite) TestKernel2Kernel_Vfio2Noop() {
	...
	r.Run(`kubectl apply -k ../../../examples/use-cases/Kernel2Kernel_Vfio2Noop`)
	...
	r.Run(`NSC_KERNEL=$(kubectl get pods -l app=alpine -n ns-kernel2kernel-vfio2noop --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
...
func (s *Suite) TestSriovKernel2Noop() {
	...
	r.Run(`kubectl apply -k ../../../examples/use-cases/SriovKernel2Noop/ponger`)
	r.Run(`kubectl -n ns-sriov-kernel2noop wait --for=condition=ready --timeout=1m pod -l app=ponger`)
	r.Run(`PONGER=$(kubectl -n ns-sriov-kernel2noop get pods -l app=ponger --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	r.Run(`kubectl -n ns-sriov-kernel2noop exec ${PONGER} -- ip a | grep "172.16.1.100"`)
	r.Run(`kubectl apply -k ../../../examples/use-cases/SriovKernel2Noop`)
...
func (s *Suite) TestVfio2Noop() {
	...
	r.Run(`kubectl apply -k ../../../examples/use-cases/Vfio2Noop`)
